...
int main(int argc, char *argv[])
{
    ...
    if(argc == 4) {
        mode = argv[1];
        data = CsvParser::parseCSV(argv[2]);
        averages = CsvParser::parseCSV(argv[3]);
    } else {
        std::cout << "invalid amount of args\tExample: 2d pathToData pathToAverages" << std::endl;
        return 0;
    }

    QApplication a(argc, argv);
    if (mode == "2d") {
        std::cout << "Mode = 2d" << std::endl;
        MainWindow w(0, data, averages);
        w.show();
        return a.exec();
    } else if(mode == "3d") {
        std::cout << "Mode = 3d" << std::endl;
        Q3DBars *widgetgraph = new Q3DBars();
        std::cout << "widgetgraph" << std::endl;
        QWidget *container = QWidget::createWindowContainer(widgetgraph);
        std::cout << "container" << std::endl;
        GraphModifier *modifier = new GraphModifier(widgetgraph, data, averages);
        ...
        widget->setWindowTitle(QStringLiteral("Load Across in Nodes from Server Simulation"));
        ...
        //constructor select for slices
        QComboBox *rangeList = new QComboBox(widget);
        for (int i = 0; i < (int)data[0].size(); i++) {
            rangeList->addItem(QString::fromStdString(std::to_string(i)));
        }
        rangeList->addItem(QStringLiteral("All"));
        rangeList->setCurrentIndex((int)data[0].size());
        ...
        //connect signals and slots for graph modifier
        ...
        std::cout << "Mode not recognized. Please specify mode as either 2d or 3d" << std::endl;
        return 0;
    }
}

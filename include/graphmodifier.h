...
class GraphModifier : public QObject
{
    Q_OBJECT
public:
    explicit GraphModifier(Q3DBars *bargraph, std::vector<std::vector<std::string>> data,
                           std::vector<std::vector<std::string>> averages);
    QStringList getNodeAxisList();
    QStringList getTimeAxisList();
    ~GraphModifier();

    void setLoadData();
    void setLoadRange(int min, int max);
...
private:
    ...
    float m_minval;
    float m_maxval;
    QValue3DAxis *m_loadAxis;
    QCategory3DAxis *m_timeAxis;
    QCategory3DAxis *m_nodeAxis;
